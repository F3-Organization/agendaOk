  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <Card variant="glass" className="w-full max-w-lg p-8 max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-bold mb-6">{form.id ? t('admin.plans.editPlan') : t('admin.plans.newPlan')}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            {!form.id && field(t('admin.plans.slug'), 'slug')}
            {field(t('admin.plans.name'), 'name')}
          </div>
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">{t('admin.plans.description')}</label>
            <textarea
          ...
          <div className="grid grid-cols-2 gap-4">
            {field(t('admin.plans.priceCents'), 'priceInCents', 'number')}
            {field(t('admin.plans.maxDevices'), 'maxDevices', 'number')}
          </div>
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">
              {t('admin.plans.messageLimit')}
            </label>
            <input
              type="number"
          ...
          {field(t('admin.plans.sortOrder'), 'sortOrder', 'number')}
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">
              {t('admin.plans.featuresPerLine')}
            </label>
            <textarea
              value={featuresText}
          ...
          <div className="flex gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm font-medium">{t('admin.plans.active')}</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm font-medium">{t('admin.plans.purchasable')}</span>
            </label>
          </div>
          <div className="flex gap-3 pt-2">
            ...
            <Button type="button" variant="secondary" onClick={onClose} disabled={isSaving}>
              <X className="w-4 h-4 mr-2" /> {t('admin.plans.cancel')}
            </Button>
          </div>
        </form>
      </Card>
...
export const AdminPlansPage = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-plans'] }); toast.success(t('admin.plans.created')); setEditingPlan(null); },
    onError: (e: any) => toast.error(e.response?.data?.error || t('admin.plans.createError')),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, ...data }: Plan) => adminService.updatePlan(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-plans'] }); toast.success(t('admin.plans.updated')); setEditingPlan(null); },
    onError: (e: any) => toast.error(e.response?.data?.error || t('admin.plans.updateError')),
  });

  const deleteMutation = useMutation({
    mutationFn: adminService.deletePlan,
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-plans'] }); toast.success(t('admin.plans.deleted')); setConfirmDelete(null); },
    onError: (e: any) => toast.error(e.response?.data?.error || t('admin.plans.deleteError')),
  });
    ...
    <AdminLayout>
      <div className="flex items-center justify-between mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <Button variant="primary" onClick={() => setEditingPlan(EMPTY_PLAN)}>
          <Plus className="w-4 h-4 mr-2" /> {t('admin.plans.newPlan')}
        </Button>
      </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    ...
                    {!plan.isActive && <span className="text-xs bg-red-500/10 text-red-400 border border-red-500/20 px-2 py-0.5 rounded font-bold uppercase tracking-widest">{t('admin.plans.inactive')}</span>}
                    {plan.isPurchasable && <span className="text-xs bg-primary/10 text-primary border border-primary/20 px-2 py-0.5 rounded font-bold uppercase tracking-widest flex items-center gap-1"><ShieldCheck className="w-3 h-3" /> {t('admin.plans.purchasable')}</span>}
                  </div>
                  {plan.description && <p className="text-sm text-muted-foreground mb-3">{plan.description}</p>}
                  <div className="flex flex-wrap gap-4 text-sm">
                    <span className="flex items-center gap-1 font-bold">
                      <DollarSign className="w-4 h-4 text-primary" />
                      {plan.priceInCents === 0 ? t('admin.plans.free') : `R$ ${(plan.priceInCents / 100).toFixed(2)}${t('admin.plans.perMonth')}`}
                    </span>
                    <span className="text-muted-foreground">
                      {plan.messageLimit === null ? `∞ ${t('admin.plans.messages')}` : `${plan.messageLimit} ${t('admin.plans.msgPerMonth')}`}
                    </span>
                    <span className="text-muted-foreground">{plan.maxDevices} {t('admin.plans.devices')}</span>
                  </div>
