export const pt = {
  sidebar: {
    sections: {
      overview: 'Visão geral',
      tools: 'Ferramentas',
      account: 'Conta',
    },
  },
  common: {
    dashboard: 'Painel',
    appointments: 'Agendamentos',
    whatsapp: 'WhatsApp',
    subscription: 'Assinatura',
    settings: 'Configurações',
    signOut: 'Sair',
    storageUsage: 'USO DE ARMAZENAMENTO',
    used: 'usado de',
    welcomeBack: 'Bem-vindo de volta',
    pleaseEnterDetails: 'Por favor, insira seus dados para continuar',
    email: 'E-mail',
    password: 'Senha',
    signIn: 'Entrar',
    forgotPassword: 'Esqueceu a senha?',
    dontHaveAccount: 'Não tem uma conta?',
    requestAccess: 'Solicitar Acesso',
    privacyPolicy: 'Política de Privacidade',
    termsOfService: 'Termos de Serviço',
    edit: 'Editar',
    delete: 'Excluir',
    viewAll: 'Ver tudo',
    confirmed: 'Confirmado',
    pending: 'Pendente',
    cancelled: 'Cancelado',
    cancel: 'Cancelar',
    connect: 'Conectar',
    contactSales: 'Falar com Vendas',
    currentPlan: 'Plano Atual',
    activePlan: 'PLANO ATIVO',
    completePayment: 'Concluir Pagamento',
    fullName: 'Nome Completo',
    signUp: 'Cadastrar',
    alreadyHaveAccount: 'Já tem uma conta?',
    orContinueWith: 'Ou continue com',
    registrationFailed: 'Falha no registro. Por favor, tente novamente.',
    loginFailed: 'Falha no login. Verifique suas credenciais.',
    joinConfirmaZap: 'Junte-se ao ConfirmaZap para automatizar seus agendamentos.',
    createAccount: 'Criar Conta',
    verifyEmail: 'Verifique seu E-mail',
    verificationCode: 'Código de Verificação',
    verificationCodeSent: 'Enviamos um código de 6 dígitos para',
    verifyAndComplete: 'Verificar e Completar',
    didntReceiveCode: 'Não recebeu o código?',
    backToLogin: 'Voltar ao login',
    verificationFailed: 'Falha na verificação. Por favor, verifique o código.',
    newPassword: 'Nova Senha',
    retry: 'Tentar Novamente',
    disconnect: 'Desconectar',
  },

  login: {
    heroTitle: 'Automatize seus agendamentos sem esforço.',
    heroSubtitle: 'A camada de curadoria premium para a comunicação do seu negócio. Experimente agendamentos sem atrito com fluxos automatizados de confirmação pelo WhatsApp.',
    trustedBy: '',
    v2Live: 'ESTA É UMA VERSÃO BETA',
    limitedAccess: 'BETA DE ACESSO LIMITADO',
    automateWhatsApp: 'Automatize o',
    engagement: 'Engajamento',
    heroDescription: 'A ferramenta definitiva para profissionais liberais e prestadores de serviços automatizarem mensagens de confirmação e reduzirem faltas.',
  },
  dashboard: {
    title: 'Visão Geral',
    subtitle: 'Gerencie suas confirmações automáticas de agendamentos e acompanhe o desempenho da comunicação.',
    deleteConfirmation: 'Tem certeza que deseja excluir permanentemente este agendamento?',
    stats: {
      confirmations: 'Total de Confirmações Env.',
      conversionRate: 'Taxa de Conversão',
      replies: 'Respostas Gerenciadas',

    },

    recentAppointments: 'Agendamentos Recentes',
    table: {
      patient: 'Paciente/Cliente',
      schedule: 'Horário',
      status: 'Status',
    },
    loadingSchedule: 'Carregando sua agenda...',
    failedToLoad: 'Falha ao carregar agendamentos.',
    noAppointments: 'Nenhum agendamento encontrado. Sincronize seu calendário para começar.',
    filters: {
      search: 'Buscar por nome ou telefone...',
      status: 'Todos os Status',
      confirmed: 'Confirmados',
      pending: 'Pendentes',
      cancelled: 'Cancelados',
    },
    appointmentDetails: {
      title: 'Detalhes do Agendamento',
      clientName: 'Nome do Cliente',
      clientPhone: 'Telefone do Cliente',
      startAt: 'Data e Hora de Início',
      endAt: 'Data e Hora de Término',
      participants: 'Participantes',
      status: {
        accepted: 'Aceitou',
        declined: 'Recusou',
        tentative: 'Talvez',
        needsAction: 'Pendente'
      },
      close: 'Fechar'
    },
    newAppointment: {
      button: 'Novo Agendamento',
      title: 'Agendar Horário',
      editTitle: 'Editar Agendamento',
      serviceName: 'Nome do Serviço',
      clientName: 'Nome do Cliente',
      clientPhone: 'Telefone do Cliente (WhatsApp)',
      startAt: 'Data e Hora de Início',
      endAt: 'Data e Hora de Fim',
      cancel: 'Cancelar',
      submit: 'Salvar Agendamento',
      success: 'Agendamento criado com sucesso no Google Agenda!',
      error: 'Erro ao criar agendamento.',
      creating: 'Salvando...'
    },
    onboarding: {
      phoneTitle: 'Complete o perfil da Empresa',
      phoneDescription: 'Para a empresa receber notificações e alertas de agendamentos, precisamos do número de WhatsApp da empresa com DDD.',
      phonePlaceholder: 'Ex: 11999999999',
      phoneButton: 'Salvar Telefone',
      phoneSuccess: 'Telefone salvo com sucesso!'
    },
    bot: {
      title: 'Bot de Atendimento',
      subtitle: 'Configure seu assistente virtual',
      description: 'Configure as respostas automáticas, horários de atendimento e instruções do seu bot para oferecer a melhor experiência aos seus clientes.',
      button: 'Configurar Bot',
    },
    whatsapp: {
      title: 'WhatsApp',
      description: 'Conecte seu WhatsApp para começar a receber e responder mensagens automaticamente com seu bot de IA.',
      button: 'Gerenciar WhatsApp',
    },
  },
  whatsapp: {
    title: 'Conexão WhatsApp',
    subtitle: 'Conecte sua conta comercial do WhatsApp para começar a automatizar as confirmações.',
    stepsTitle: 'Siga estes passos',
    steps: [
      'Abra o WhatsApp no seu celular',
      'Toque em Menu ou Configurações e selecione Aparelhos Conectados',
      'Aponte seu celular para esta tela para capturar o código',
    ],
    security: {
      title: 'Criptografado de Ponta a Ponta',
      description: 'O ConfirmaZap nunca armazena suas mensagens ou informações de contato pessoais. Apenas usamos a conexão para enviar templates automatizados.',
    },
    help: 'Precisa de ajuda?',
    waitingScan: 'Aguardando leitura',
    expiresIn: 'O código QR expira em',
    regenerate: 'Regenerar Código',
    scanWithPhone: 'Abra o WhatsApp no celular e escaneie o código acima.',
    clickToRefresh: 'Clique para Atualizar',
    generatingQR: 'Gerando QR...',
    qrCodeAlt: 'Código QR do WhatsApp',
    failedLoadQR: 'Falha ao carregar QR',
    connected: 'Conectado',
    connectButton: 'Conectar WhatsApp',
    successTitle: 'WhatsApp Conectado!',
    successDescription: 'Sua conta foi pareada com sucesso. Agora você pode automatizar suas confirmações de agendamento.',
    startAutomating: 'Ir para o Dashboard',
    config: {
      title: 'Configurações de Notificação',
      silentWindow: 'Janela de Silêncio',
      silentWindowDescription: 'Defina o período em que o sistema NÃO deve enviar mensagens de confirmação automaticamente para evitar incomodar seus clientes em horários inadequados.',
      enabled: 'Ativado',
      start: 'Horário de Início',
      end: 'Horário de Fim',
      save: 'Salvar Configurações',
      success: 'Configurações salvas com sucesso!'
    }
  },
  subscription: {
    title: 'Plano e Faturamento',
    subtitle: 'Gerencie sua assinatura, veja o histórico de faturamento e faça upgrade para desbloquear mais recursos.',
    plans: {
      standard: {
        name: 'Padrão',
        description: 'Perfeito para profissionais liberais e pequenas empresas',
      },
      pro: {
        name: 'Profissional',
        description: 'Recursos avançados para equipes em crescimento',
      },
      enterprise: {
        name: 'Enterprise',
        description: 'Soluções escaláveis para grandes redes e operações',
        customPrice: 'Personalizado',
      }
    },
    features: {
      monthlyConfirmations: 'confirmações mensais',
      unlimitedConfirmations: 'Confirmações ilimitadas',
      whatsappDevices: 'dispositivo WhatsApp',
      whatsappDevicesPlural: 'dispositivos WhatsApp',
      support: 'Suporte por e-mail',
      prioritySupport: 'Suporte prioritário 24/7',
      reporting: 'Relatórios básicos',
      apiAccess: 'Acesso avançado à API',
      branding: 'Marca personalizada',
      dedicatedManager: 'Gerente de conta dedicado',
      onPremise: 'Implantação local',
      hipaa: 'Conformidade HIPAA',
    },
    billing: {
      title: 'Histórico de Faturamento',
      recentInvoices: 'Faturas Recentes',
      subtitle: 'Baixe seus registros mensais de transações',
      invoiceId: 'ID da Fatura',
      date: 'Data',
      amount: 'Valor',
      status: 'Status',
      requestStatement: 'Solicitar Extrato',
      paid: 'Pago',
      downloadPdf: 'PDF',
      loadingStatus: 'Carregando informações de cobrança...',
      loadingHistory: 'Buscando histórico...',
      noPayments: 'Nenhum pagamento encontrado',
      noPaymentsDescription: 'Você ainda não possui faturas ou assinaturas registradas no sistema.',
      pendingTitle: 'Pagamento em Processamento',
      pendingDescription: 'Sua assinatura PRO será ativada assim que confirmarmos o recebimento do pagamento. Isso pode levar alguns minutos.',
      successTitle: 'Plano PRO Ativado!',
      successDescription: 'Seu pagamento foi confirmado. Agora você tem acesso ilimitado a todos os recursos.',
      infoSaved: 'Dados de faturamento salvos!',
    },
    billingModal: {
      title: 'Dados de Faturamento',
      description: 'Para migrar para o plano Pro, precisamos de alguns dados adicionais para a emissão das faturas.',
      taxId: 'CPF ou CNPJ (apenas números)',
      taxIdPlaceholder: 'Ex: 12345678901',
      taxIdError: 'CPF deve ter 11 dígitos ou CNPJ 14 dígitos',
      taxIdNumbersOnly: 'Apenas números',
      whatsapp: 'WhatsApp (com DDD)',
      whatsappPlaceholder: 'Ex: 11999999999',
      whatsappError: 'Número inválido',
      saveButton: 'Salvar e Continuar',
      cancelButton: 'Cancelar',
    },
    pricing: {
      perMonth: '/mês',
    },
    usage: {
      title: 'Uso do Plano',
      messageLimit: 'Limite de Mensagens',
      sent: 'enviadas',
      limitReached: 'Limite atingido!',
      limitReachedTitle: 'Limite de Mensagens Atingido',
      limitReachedDesc: 'Sua cota mensal de mensagens foi atingida. Faça o upgrade para PRO para continuar confirmando agendamentos.',
      limitWarningTitle: 'Você tem apenas {{count}} mensagens restantes',
      limitWarningDesc: 'Seu limite de mensagens mensais está quase no fim. Não deixe seus agendamentos sem confirmação!',
      upgradeButton: 'Migrar para PRO agora',
    },
    enterpriseCta: {
      title: 'Precisa de uma solução enterprise personalizada?',
      description: 'Oferecemos preços sob medida para operações de grande escala com várias unidades e milhares de confirmações diárias.',
      button: 'Agendar Demo',
    },
    enterpriseMessage: 'Olá, gostaria de saber mais sobre o plano Enterprise do ConfirmaZap',
    trial: {
      bannerTitle: 'Você está no período de teste gratuito',
      bannerDescription: 'Aproveite todos os recursos PRO! Seu trial expira em {{days}} dia(s).',
      daysLeft: 'dias restantes',
      activatedTitle: 'Trial PRO Ativado!',
      currentPlan: 'Em Trial',
    },
    pix: {
      title: 'Pagar com PIX',
      copyLabel: 'PIX Copia e Cola',
      copy: 'Copiar',
      copied: 'Copiado!',
      instructions: 'Abra seu app de pagamento, escaneie o QR Code ou cole o código acima. O plano PRO será ativado automaticamente após a confirmação.',
      paid: 'Pagamento Confirmado!',
      paidDescription: 'Ativando seu plano PRO. Aguarde um instante...',
      payWithPix: 'Pagar com PIX',
      payWithCard: 'Pagar com Cartão',
    },
    cancel: {
      button: 'Cancelar Assinatura',
      confirmTitle: 'Cancelar Assinatura',
      confirmDescription: 'Tem certeza que deseja cancelar? Você perderá o acesso aos recursos PRO no final do período atual.',
      confirm: 'Sim, Cancelar',
      success: 'Assinatura cancelada com sucesso.',
    },
  },
  landing: {
    heroTitle: 'Sua recepcionista IA no WhatsApp. 24/7.',
    heroSubtitle: 'Atenda pacientes, agende consultas e confirme horários automaticamente pelo WhatsApp — sem contratar ninguém. Inteligência artificial que trabalha enquanto você descansa.',
    ctaStartNow: 'Começar Agora',
    ctaNoCard: 'Comece grátis — sem compromisso',
    featuresTitle: 'Tudo que sua clínica precisa. Em um só lugar.',
    featuresSubtitle: 'Do primeiro "oi" ao lembrete final — o bot cuida de tudo.',
    aiBotTitle: 'Bot IA Inteligente',
    aiBotDesc: 'Responde dúvidas, agenda horários e conversa com seus pacientes como uma recepcionista real. Funciona 24h, 7 dias por semana.',
    calendarSyncTitle: 'Sincronização com Google Calendar',
    calendarSyncDesc: 'Agenda atualizada em tempo real. Quando o bot marca uma consulta, ela aparece no seu calendário automaticamente.',
    whatsappAutoTitle: 'Confirmação Automática',
    whatsappAutoDesc: 'Reduza faltas em até 70%. Envie lembretes inteligentes e receba confirmações sem precisar ligar para ninguém.',
    dashboardTitle: 'Dashboard em Tempo Real',
    dashboardDesc: 'Acompanhe atendimentos, taxas de confirmação e performance do bot com métricas claras e visuais.',
    pricingTitle: 'Preços Simples e Transparentes',
    pricingSubtitle: 'Comece grátis. Quando crescer, o PRO custa menos que um café por dia.',
    aiBotSectionTitle: 'Seu atendente nunca tira folga.',
    aiBotSectionSubtitle: 'O Bot IA do ConfirmaZap entende contexto, responde perguntas sobre seus serviços, verifica disponibilidade e agenda consultas — tudo direto no WhatsApp.',
    aiBotFeature1Title: 'Entende Linguagem Natural',
    aiBotFeature1Desc: 'Pacientes conversam normalmente. O bot entende "quero marcar terça" ou "tem horário pra amanhã?" sem botões ou menus.',
    aiBotFeature2Title: 'Conhece Seu Negócio',
    aiBotFeature2Desc: 'Configurado com seus serviços, profissionais, horários e instruções. Cada resposta é personalizada para a sua clínica.',
    aiBotFeature3Title: 'Funciona 24/7',
    aiBotFeature3Desc: 'Pacientes entram em contato fora do horário? O bot responde e agenda. Nenhuma oportunidade é perdida.',
    plans: {
      free: 'Grátis',
      freeDesc: 'Perfeito para começar.',
      pro: 'Pro',
      proDesc: 'Escalone seu serviço profissional.',
      remindersPerMonth: '50 Lembretes /mês',
      unlimitedReminders: 'Lembretes Ilimitados',
      customTemplates: 'Modelos de WhatsApp Personalizados',
      prioritySupport: 'Suporte Prioritário 24/7',
      aiBotFeature: 'Bot IA com autoatendimento',
      professionalMgmt: 'Gestão de profissionais e horários',
      multiCompany: 'Até 3 empresas por conta',
    },
    footerProduct: 'Produto',
    footerLegal: 'Legal',
    footerSupport: 'Suporte',
    multiTenantTitle: 'Feito para Clínicas e Consultórios',
    multiTenantDesc: 'Gerencie múltiplos profissionais, cada um com seus horários e especialidades. Dados isolados por empresa com segurança total.',
  },
  settings: {
    title: 'Configurações',
    subtitle: 'Gerencie seus dados de perfil e preferências do sistema',
    profile: {
      title: 'Perfil Profissional',
      description: 'Atualize suas informações de contato e detalhes fiscais para garantir que sua conta e faturas estejam sempre corretas.',
      fullName: 'Nome Completo',
      email: 'E-mail',
      taxId: 'CPF / CNPJ',
      whatsapp: 'WhatsApp de Contato',
      whatsappHint: 'Para alertas administrativos e notificações de sistema.',
      placeholders: {
        fullName: 'Seu nome completo',
        email: 'exemplo@email.com',
        taxId: '000.000.000-00',
        whatsapp: '11999999999',
      }
    },
    automation: {
      title: 'Automação & Silêncio',
      description: 'Controle como e quando o ConfirmaZap interage com seus clientes. Ative a sincronização e defina janelas de descanso para mantê-lo produtivo sem interrupções indesejadas.',
      syncTitle: 'Sincronização Ativa',
      syncDesc: 'Habilite ou desabilite a automação total com o Google Calendar',
      silentWindow: 'Janela de Silêncio',
      silentWindowDesc: 'Defina o horário em que o sistema NÃO enviará mensagens automáticas pelo WhatsApp para seus clientes.',
      start: 'Início',
      end: 'Término',
    },
    security: {
      title: 'Segurança & Privacidade',
      description: 'Proteja sua conta com camadas extras de defesa. Gerencie sua senha e ative a autenticação em duas etapas para garantir que apenas você tenha acesso aos seus dados de agendamento.',
      changePassword: 'Alterar Senha',
      setPassword: 'Definir Senha',
      currentPassword: 'Senha Atual',
      newPassword: 'Nova Senha',
      confirmPassword: 'Confirmar Nova Senha',
      passwordPlaceholder: '••••••••',
      newPasswordHint: 'Mínimo 6 caracteres',
      updatePassword: 'Atualizar Senha',
      twoFactor: 'Autenticação em Duas Etapas',
      twoFactorActive: 'Ativo',
      twoFactorInactive: 'Desativado',
      twoFactorStatus: 'Status do 2FA',
      twoFactorDesc: 'Adicione uma camada extra de segurança à sua conta. Ao ativar, você precisará de um código do seu aplicativo autenticador para fazer login.',
      setupScanner: 'Configurar App Autenticador',
      centerTitle: 'Central de Segurança ConfirmaZap',
    },
    messages: {
      saveSuccess: 'Configurações salvas com sucesso!',
      saveError: 'Erro ao salvar configurações',
      passwordSuccess: 'Senha alterada com sucesso!',
      passwordError: 'Erro ao alterar senha',
      passwordMismatch: 'As senhas não coincidem',
      twoFactorEnabled: '2FA habilitado com sucesso!',
      twoFactorDisabled: '2FA desabilitado.',
      twoFactorError: 'Erro ao configurar 2FA',
      protectedData: 'Seus dados estão protegidos e criptografados',
      savePreferences: 'Salvar Preferências',
      saving: 'Salvando...',
    },
    twoFactorLogin: {
      title: 'Verificação em Duas Etapas',
      subtitle: 'Digite o código de 6 dígitos gerado pelo seu aplicativo autenticador para confirmar sua identidade.',
      verifyButton: 'Verificar e Entrar',
      invalidCode: 'Código inválido ou expirado. Tente novamente.',
      placeholder: '000000',
    }
  },
  company: {
    select: {
      title: 'Selecione sua Empresa',
      subtitle: 'Escolha a empresa que deseja acessar',
      newCompany: 'Nova Empresa',
      limitReached: 'Limite de empresas atingido no plano atual',
    },
    create: {
      title: 'Criar Empresa',
      subtitle: 'Crie sua empresa para começar a usar o ConfirmaZap',
      nameLabel: 'Nome da Empresa',
      namePlaceholder: 'Ex: Clínica Dr. Silva',
      nameMinLength: 'O nome deve ter pelo menos 2 caracteres',
      submitButton: 'Criar Empresa',
      back: 'Voltar para seleção de empresa',
      error: 'Erro ao criar empresa. Tente novamente.',
    },
    switcher: {
      newCompany: 'Nova Empresa',
    },
    settings: {
      title: 'Configurações da Empresa',
      subtitle: 'Gerencie as configurações da sua empresa',
      navLabel: 'Empresa',
      islandTitle: 'Configurações da Empresa',
      identity: {
        title: 'Identidade',
        description: 'Nome e informações básicas da empresa.',
        nameLabel: 'Nome da Empresa',
      },
      whatsapp: {
        title: 'WhatsApp',
        description: 'Configure o número de WhatsApp e as janelas de silêncio para esta empresa.',
        numberLabel: 'Número do WhatsApp',
      },
      billing: {
        title: 'Cobrança',
        description: 'Dados fiscais e informações do plano desta empresa.',
        planLabel: 'Plano Atual',
      },
      danger: {
        title: 'Zona de Perigo',
        description: 'Ações irreversíveis. Tenha cuidado.',
        deleteTitle: 'Excluir Empresa',
        deleteDescription: 'Todos os dados, agendamentos e configurações serão perdidos permanentemente.',
        deleteButton: 'Excluir',
        confirmDelete: 'Confirmar Exclusão',
      },
      messages: {
        saveSuccess: 'Configurações salvas com sucesso!',
        saveError: 'Erro ao salvar configurações',
        renameSuccess: 'Nome atualizado com sucesso!',
        renameError: 'Erro ao renomear',
        deleteError: 'Erro ao excluir empresa',
      },
    },
  },
  checkout: {
    title: 'Revisar seu Pedido',
    subtitle: 'Você está a um passo de desbloquear o potencial máximo do seu negócio no WhatsApp.',
    backButton: 'Voltar aos planos',
    selectedPlan: 'PLANO SELECIONADO',
    perMonth: 'por mês',
    perYear: 'por ano',
    unlimitedNotifications: 'Notificações Ilimitadas no WhatsApp',
    googleSync: 'Sincronização com Google Calendar',
    fiscalAutomation: 'Automação Fiscal (NFS-e FocusNFe)',
    confirmButton: 'Confirmar e Pagar com Segurança',
    loadingButton: 'Preparando Checkout...',
    sslSecure: 'SSL Seguro',
    cloudEncrypted: 'Criptografia em Nuvem',
    recurringBilling: 'Cobrança Recorrente',
    nextSteps: {
        title: 'O que acontece agora?',
        activationTitle: 'Ativação Instantânea',
        activationDesc: 'Assim que o pagamento for confirmado via AbacatePay, seu plano Pro será ativado na hora.',
        syncTitle: 'Sincronização Automática',
        syncDesc: 'Seus próximos agendamentos começarão a ser confirmados magicamente no WhatsApp.',
        invoiceTitle: 'Nota Fiscal Automática',
        invoiceDesc: 'Receba sua NFS-e por e-mail em cada renovação, sem burocracia.',
    },
    securePaymentFooter: 'Pagamento Seguro via',
    paymentCreditCard: 'Cartão',
    paymentPix: 'PIX',
    footerNote: '*Cobrança recorrente automática processada com segurança. Aceitamos Cartão de Crédito e PIX para sua comodidade.'
  },
  admin: {
    dashboard: {
      title: 'Dashboard',
      subtitle: 'Visão geral da plataforma ConfirmaZap',
      totalUsers: 'Total de Usuários',
      totalCompanies: 'Total de Empresas',
      professionals: 'Profissionais',
      appointments: 'Agendamentos',
      estimatedMRR: 'Receita Estimada (MRR)',
      basedOnPro: 'Baseado em assinaturas ativas PRO',
      activeProSubs: '{{count}} assinaturas PRO ativas',
      growth30d: 'Crescimento (30 dias)',
      newRegistrations: 'Novos registros por dia',
      registrations: 'registros',
      daysAgo: '30 dias atrás',
      today: 'Hoje',
      subscriptionsByPlan: 'Assinaturas por Plano',
    },
    users: {
      title: 'Usuários',
      registered: '{{count}} usuários cadastrados',
      searchPlaceholder: 'Buscar por nome ou email...',
      search: 'Buscar',
      user: 'Usuário',
      plan: 'Plano',
      companies: 'Empresas',
      auth: 'Auth',
      registration: 'Registro',
      actions: 'Ações',
      details: 'Detalhes',
      impersonate: 'Impersonar',
      page: 'Página {{page}} de {{totalPages}}',
      previous: 'Anterior',
      next: 'Próxima',
      role: 'Role',
      twoFactorActive: 'Ativo',
      twoFactorInactive: 'Inativo',
      changePlan: 'Alterar Plano',
      cancel: 'Cancelar',
      save: 'Salvar',
    },
    companies: {
      title: 'Empresas',
      registered: '{{count}} empresas cadastradas',
      searchPlaceholder: 'Buscar por nome da empresa, dono ou email...',
      search: 'Buscar',
      company: 'Empresa',
      owner: 'Proprietário',
      plan: 'Plano',
      professionals: 'Profissionais',
      createdAt: 'Criada em',
      page: 'Página {{page}} de {{totalPages}}',
      previous: 'Anterior',
      next: 'Próxima',
    },
    plans: {
      title: 'Planos',
      subtitle: 'Gerencie os planos de assinatura disponíveis.',
      newPlan: 'Novo Plano',
      editPlan: 'Editar Plano',
      slug: 'Slug (ex: PRO)',
      name: 'Nome',
      description: 'Descrição',
      priceCents: 'Preço (centavos)',
      maxDevices: 'Máx. Dispositivos',
      messageLimit: 'Limite de Mensagens (vazio = ilimitado)',
      unlimited: 'Ilimitado',
      sortOrder: 'Ordem (sortOrder)',
      featuresPerLine: 'Features (uma por linha)',
      active: 'Ativo',
      inactive: 'Inativo',
      purchasable: 'Comprável (gera checkout)',
      save: 'Salvar',
      cancel: 'Cancelar',
      free: 'Gratuito',
      perMonth: '/mês',
      messages: 'mensagens',
      msgPerMonth: 'msg/mês',
      devices: 'dispositivo(s)',
      created: 'Plano criado!',
      updated: 'Plano atualizado!',
      deleted: 'Plano removido!',
      createError: 'Erro ao criar plano',
      updateError: 'Erro ao atualizar plano',
      deleteError: 'Erro ao remover plano',
    },
    nav: {
      dashboard: 'Dashboard',
      users: 'Usuários',
      companies: 'Empresas',
      plans: 'Planos',
    },
  },
  googleCallback: {
    authenticating: 'Autenticando...',
    pleaseWait: 'Aguarde enquanto configuramos sua sessão.',
  },
  professionals: {
    title: 'Profissionais',
    subtitle: 'Gerencie os profissionais da sua empresa',
    newProfessional: 'Novo Profissional',
    editProfessional: 'Editar Profissional',
    save: 'Salvar',
    create: 'Criar',
    activate: 'Ativar',
    deactivate: 'Desativar',
    edit: 'Editar',
    delete: 'Excluir',
    empty: 'Nenhum profissional cadastrado ainda.',
    registered: '{{count}} cadastrados',
    durationPerAppointment: '{{duration}} min por consulta',
    accountLinked: 'Conta vinculada',
    inviteSent: 'Convite enviado',
    resendChange: 'Reenviar / Alterar',
    linkAccount: 'Vincular Conta',
    inviteError: 'Erro ao vincular conta.',
    deleteTitle: 'Excluir Profissional',
    deleteDescription: 'Tem certeza que deseja excluir este profissional? Esta ação não pode ser desfeita.',
    form: {
      name: 'Nome',
      namePlaceholder: 'Dr. João Silva',
      specialty: 'Especialidade',
      specialtyPlaceholder: 'Ex: Dentista, Barbeiro, Personal...',
      duration: 'Duração da Consulta (minutos)',
      workingHours: 'Horários de Trabalho',
      shift: 'Turno',
      to: 'até',
    },
    invite: {
      title: 'Vincular Conta de Acesso',
      description: 'O profissional receberá um email para criar sua senha e acessar a agenda. Você pode reenviar ou alterar o email a qualquer momento.',
      emailLabel: 'Email do Profissional',
      emailPlaceholder: 'profissional@email.com',
      send: 'Enviar Convite',
      success: 'Email de verificação enviado com sucesso!',
    },
    days: {
      mon: 'Segunda',
      tue: 'Terça',
      wed: 'Quarta',
      thu: 'Quinta',
      fri: 'Sexta',
      sat: 'Sábado',
      sun: 'Domingo',
    },
  },
  botConfig: {
    title: 'Configuração do Bot',
    subtitle: 'Personalize o bot de atendimento da sua empresa no WhatsApp.',
    enable: {
      title: 'Bot de Autoatendimento',
      description: 'Quando ativado, o bot responderá automaticamente mensagens dos clientes no WhatsApp usando inteligência artificial.',
    },
    plan: {
      upgradeRequired: 'O Bot IA é exclusivo do plano PRO. Faça upgrade para ativar.',
      proRequired: 'Recurso exclusivo do Plano PRO',
      proDescription: 'O Bot de Autoatendimento com Inteligência Artificial está disponível apenas no plano PRO. Faça upgrade para ativar respostas automáticas, atendimento 24h e muito mais.',
      upgrade: 'Fazer upgrade para PRO',
    },
    prerequisites: {
      title: 'Pré-requisitos para ativar o bot',
      subtitle: 'Para ativar o bot de autoatendimento, é necessário configurar os seguintes itens:',
      cannotEnable: 'Configure os itens obrigatórios antes de ativar o bot.',
      cannotSave: 'Preencha os pré-requisitos obrigatórios antes de ativar o bot.',
      whatsappNumber: 'Número de WhatsApp da empresa',
      businessDescription: 'Descrição do negócio',
      configure: 'Configurar',
    },
    business: {
      title: 'Detalhes da Empresa',
      description: 'Essas informações serão usadas pelo bot para responder perguntas sobre sua empresa.',
    },
    services: {
      title: 'Serviços Oferecidos',
      description: 'Liste os serviços que sua empresa oferece. O bot usará isso para orientar os clientes.',
    },
    personality: {
      title: 'Personalidade do Bot',
      description: 'Defina como o bot se comunica com seus clientes.',
    },
    businessTypes: {
      clinic: 'Clínica / Consultório',
      salon: 'Salão / Barbearia',
      studio: 'Estúdio / Academia',
      office: 'Escritório / Coworking',
      restaurant: 'Restaurante',
      other: 'Outro',
    },
  },
  appointmentsPage: {
    subtitle: 'Gerencie os agendamentos da sua empresa',
    subtitleProfessional: 'Sua agenda de atendimentos',
    searchPlaceholder: 'Buscar por nome, telefone...',
    filterAll: 'Todos',
    filterDateFrom: 'De',
    filterDateTo: 'Até',
    allProfessionals: 'Todos profissionais',
    statuses: {
      pending: 'Pendente',
      confirmed: 'Confirmado',
      cancelled: 'Cancelado',
    },
    emptyTitle: 'Nenhum agendamento encontrado.',
    emptyFilters: 'Tente ajustar os filtros.',
    emptyNone: 'Você não tem agendamentos ainda.',
    emptyHint: 'Clique em "Novo Agendamento" para começar.',
    editAppointment: 'Editar Agendamento',
    newAppointment: 'Novo Agendamento',
    save: 'Salvar',
    create: 'Criar',
    notified: 'Notificado',
    notNotified: 'Não notificado',
    deleteTitle: 'Excluir agendamento?',
    deleteDescription: 'Esta ação não pode ser desfeita.',
    deleteConfirm: 'Excluir',
    table: {
      client: 'Cliente',
      service: 'Serviço',
      professional: 'Profissional',
      dateTime: 'Data / Hora',
      status: 'Status',
      notification: 'Notif.',
    },
    form: {
      serviceTitle: 'Serviço / Título',
      servicePlaceholder: 'Ex: Consulta, Corte, Treino...',
      clientName: 'Nome do Cliente',
      clientNamePlaceholder: 'João Silva',
      clientPhone: 'WhatsApp do Cliente',
      startAt: 'Início',
      endAt: 'Fim (opcional)',
      notes: 'Observações',
      notesPlaceholder: 'Informações adicionais...',
      selectProfessional: 'Selecione um profissional',
    },
    validation: {
      titleRequired: 'Título é obrigatório',
      clientNameRequired: 'Nome do cliente é obrigatório',
      phoneRequired: 'Telefone é obrigatório',
      phoneInvalid: 'Telefone inválido (mín. 10 dígitos)',
      startRequired: 'Data de início é obrigatória',
      startInPast: 'A data não pode ser no passado',
      endBeforeStart: 'O fim deve ser após o início',
      professionalRequired: 'Profissional é obrigatório',
    },
  },
  datePicker: {
    placeholder: 'Selecionar data e hora',
    selectDate: 'Selecionar',
    time: 'Horário',
    hours: 'H',
    minutes: 'M',
  },
};
